...
// BEGIN GENERATED RISK MODULE: edit Include/GoldRiskMath.mqh or GoldSignalMath.mqh, then run scripts/build_standalone.py
...
enum ENUM_GOLD_SIGNAL_PROFILE { SIGNAL_STRICT=0, SIGNAL_BALANCED=1 };
...
input ENUM_GOLD_SIGNAL_PROFILE InpSignalProfile=SIGNAL_BALANCED;
...
input group "Balanced profile (Strict retains original rules)"
input int    InpM15EMAPeriod=50;
input double InpBalancedZoneATR=1.0; // M15 ATR, unlike the Strict profile's M5 radius.
input double InpBalancedMinBodyATR=0.4;
...
int m15_ema_handle=INVALID_HANDLE,m15_atr_handle=INVALID_HANDLE;
...
ulong warmup_skips=0,trend_skips=0,raw_sweeps=0,zone_skips=0,momentum_skips=0,setup_invalidations=0,rr_adjustments=0;
bool trend_data_ready=false;
...
bool LatestSwing(MqlRates &rates[],bool high,int newest_allowed,double &value)
{
   for(int i=newest_allowed;i<ArraySize(rates)-InpPivotWidth && i<=InpSwingLookback;i++)
      if(Pivot(rates,i,high)) { value=high ? rates[i].high : rates[i].low; return true; }
   return false;
}

int Trend(double &support,double &resistance)
{
   trend_data_ready=false;
   ...
   if(close<=0) return 0;
   int h1_direction=GoldEMADirection(close,current_ema,previous_ema);
   if(InpSignalProfile==SIGNAL_BALANCED)
   {
      double ema15,previous15;
      if(BarsCalculated(m15_ema_handle)<InpM15EMAPeriod+3 || !IndicatorValue(m15_ema_handle,1,ema15) ||
         !IndicatorValue(m15_ema_handle,2,previous15)) return 0;
      double close15=iClose(_Symbol,PERIOD_M15,1);
      if(close15<=0) return 0;
      trend_data_ready=true;
      support=ema15; resistance=ema15;
      int m15_direction=GoldEMADirection(close15,ema15,previous15);
      return h1_direction!=0 && h1_direction==m15_direction ? h1_direction : 0;
   }
   MqlRates rates[];
...
bool PrepareRetracement(bool buy,MqlRates &bar,double stop,double spread,double &entry,double &target)
{
   MqlRates rates[];
   if(!LoadRates(PERIOD_M15,rates)) return false;
   for(int attempt=0;attempt<=InpSwingLookback;attempt++)
   {
      double risk=buy ? entry-stop : stop-entry;
      if(!GoldEntryInsideCandle(buy,entry,stop,bar.low,bar.high)) return false;
      target=Price(entry+(buy ? 1 : -1)*InpRewardRisk*risk,buy);
      double nearest=0;
      ...
      if(GoldTargetHasRoom(buy,entry,target,nearest,spread)) return true;
      if(InpSignalProfile==SIGNAL_STRICT) return false;
      double adjusted=GoldEntryBeforeObstacle(buy,entry,stop,InpRewardRisk,nearest,spread,TickSize());
      if(adjusted<=0 || (buy ? adjusted>=entry : adjusted<=entry)) return false;
      entry=Price(adjusted,!buy);
   }
   return false;
}
...
void PlaceSetup(int direction,MqlRates &bar,double atr,MqlTick &tick)
{
   ...
   double original_entry=entry;
   double stop=Price(setup_extreme+(buy ? -1 : 1)*InpStopBufferATR*atr,!buy);
   double target=0;
   if(!PrepareRetracement(buy,bar,stop,tick.ask-tick.bid,entry,target))
   { rr_skips++; Report("Skip: no retracement inside confirmation candle can provide target RR before M15 obstacle, or data unavailable."); return; }
   double risk=buy ? entry-stop : stop-entry;
   ...
   if(MathAbs(entry-original_entry)>=TickSize()*0.5)
   {
      rr_adjustments++;
      Report(StringFormat("RR retracement adjusted: %.5f -> %.5f, original SL unchanged, target RR=%.2f.",original_entry,entry,InpRewardRisk));
   }
...
void ProcessBar(MqlTick &tick)
{
   ...
   if(direction==0)
   {
      if(trend_data_ready) { trend_skips++; Report("Waiting: H1 trend and M15 profile filter disagree."); }
      else { warmup_skips++; Report("Waiting: H1/M15 history or indicators still warming up."); }
      setup_direction=0; return;
   }
   aligned_bars++;
   ...
   if(setup_direction!=0)
   {
      ...
      if(direction!=setup_direction || setup_age>InpConfirmBars ||
         !GoldSweepIntact(direction>0,rates[1].low,rates[1].high,setup_extreme))
      { setup_invalidations++; setup_direction=0; }
      else
      {
         bool confirmation=GoldBreakConfirmed(direction>0,rates[1].open,rates[1].close,setup_break);
         if(confirmation)
         {
            confirmation_count++;
            double rsi=0,previous=0;
            bool rsi_ready=!InpUseRSI || (IndicatorValue(rsi_handle,1,rsi) && IndicatorValue(rsi_handle,2,previous));
            double min_body=InpSignalProfile==SIGNAL_BALANCED ? InpBalancedMinBodyATR : InpMinBodyATR;
            bool momentum=rsi_ready && GoldMomentumConfirmed(direction>0,rates[1].open,rates[1].close,atr,min_body,InpUseRSI,rsi,previous);
            if(momentum) PlaceSetup(direction,rates[1],atr,tick);
            else { momentum_skips++; Report("Skip: confirmation body or RSI8 momentum failed."); }
            setup_direction=0;
            return;
         }
      }
   ...
   if(InpSignalProfile==SIGNAL_STRICT)
   {
   ...
   {
      double level=0;
      if(!LatestSwing(rates,direction<0,2+InpPivotWidth,level))
      { Report("Waiting: no pre-confirmed M5 liquidity pivot."); return; }
      if(direction>0) low1=level; else high1=level;
   }
   bool sweep=GoldSweep(direction>0,rates[1].low,rates[1].high,rates[1].close,direction>0 ? low1 : high1);
   if(sweep) raw_sweeps++;
   double extreme=direction>0 ? rates[1].low : rates[1].high;
   double zone=direction>0 ? support : resistance;
   double zone_atr=atr,tolerance=InpM15ZoneToleranceM5ATR;
   if(InpSignalProfile==SIGNAL_BALANCED)
   {
      if(!IndicatorValue(m15_atr_handle,1,zone_atr) || zone_atr<=0)
      { Report("Waiting: M15 ATR for Balanced zone not ready."); return; }
      tolerance=InpBalancedZoneATR;
   }
   if(sweep && GoldInZone(extreme,zone,zone_atr,tolerance))
   {
      sweep_count++;
      setup_direction=direction; setup_age=0; setup_extreme=extreme;
      setup_break=InpSignalProfile==SIGNAL_BALANCED ? (direction>0 ? rates[1].high : rates[1].low) : (direction>0 ? high1 : low1);
      Report("Sweep detected in profile zone; waiting for closed-candle confirmation.");
   }
   else
   {
      if(sweep) zone_skips++;
      Report(sweep ? "Waiting: sweep outside M15 zone tolerance." : "Waiting: no liquidity sweep.");
   }
}
...
bool ValidInputs()
{
   return InpMagic>0 && InpRiskPercent>0 && InpRiskPercent<=2.0 &&
      (InpSignalProfile==SIGNAL_STRICT || InpSignalProfile==SIGNAL_BALANCED) &&
      InpM15EMAPeriod>=2 && InpM15EMAPeriod<=200 && InpBalancedZoneATR>0 && InpBalancedZoneATR<=3 &&
      InpBalancedMinBodyATR>0 && InpBalancedMinBodyATR<InpMaxRangeATR &&
...
int OnInit()
{
   ...
   m15_ema_handle=iMA(_Symbol,PERIOD_M15,InpM15EMAPeriod,0,MODE_EMA,PRICE_CLOSE);
   m15_atr_handle=iATR(_Symbol,PERIOD_M15,InpATRPeriod);
   if(ema_handle==INVALID_HANDLE || atr_handle==INVALID_HANDLE || rsi_handle==INVALID_HANDLE ||
      m15_ema_handle==INVALID_HANDLE || m15_atr_handle==INVALID_HANDLE) return INIT_FAILED;
   ...
   Print("GoldTrendSweep v1.30 initialized. SINGLE-FILE BUILD. Profile=",EnumToString(InpSignalProfile),". Risk state prefix: ",prefix,"; commission is in account currency.");
...
void OnDeinit(const int reason)
{
   ...
   {
      PrintFormat("GTS v1.30 SUMMARY: evaluated_M5_bars=%I64u aligned=%I64u sweeps=%I64u structure_breaks=%I64u order_attempts=%I64u accepted_pending=%I64u filled_entry_orders=%d (negative means history unavailable).",
      ...
      PrintFormat("GTS SIGNALS: warmup=%I64u trend_disagree=%I64u raw_sweeps=%I64u zone_rejected=%I64u momentum_rejected=%I64u expired_or_invalidated=%I64u RR_adjusted=%I64u",
         warmup_skips,trend_skips,raw_sweeps,zone_skips,momentum_skips,setup_invalidations,rr_adjustments);
   }
