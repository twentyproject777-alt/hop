// Generated by scripts/build_standalone.py; edit the shared sources instead.
#property strict
...
void PlaceSetup(int direction,MqlRates &bar,double atr,double obstacle,MqlTick &tick)
{
   ...
   if(risk<=0) { Report("Skip: invalid entry/stop distance."); return; }
   ...
   { Report("Skip: retracement limit or SL too close to market/broker stop level."); return; }
   if(buy ? obstacle<=target+tick.ask-tick.bid : obstacle>=target-(tick.ask-tick.bid))
   { Report("Skip: M15 obstacle leaves insufficient room for target RR."); return; }
   if(!CostsAcceptable(buy,entry,stop,tick))
   { Report("Skip: spread/commission/slippage exceeds cost fraction, or profit calculation unavailable."); return; }
   double budget=AvailableRiskBudget();
   ...
   { Report(StringFormat("Skip margin: required=%.2f, allowed=%.2f. Check tester leverage.",margin,AccountInfoDouble(ACCOUNT_MARGIN_FREE)*InpMaxMarginPercent/100.0)); return; }
